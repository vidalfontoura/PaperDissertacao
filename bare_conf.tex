...
% in the abstract
\begin{abstract}
	The Protein Structure Prediction (PSP) problem is one the modern most challenging problems from science and can be considered as a minimization problem.  Hence, many heuristics strategies have been applied in order to find protein structures that minimizes its free energy. However, these strategies have difficulties on finding the optimal solutions to the longer sequences of amino-acids, due, the complexity of the problem and the huge amount of local optima. The hyper-heuristics framework are usually useful in this kind of context since they try to combine different heuristics strengths into a single framework. Moreover, there is lack of works whose aim the automated design of hyper-heuristics components. This paper proposes the GEHyPDP which aims the generation, through grammatical evolution, of selection mechanisms and acceptance criteria to a hyper-heuristic framework applied to PSP problem.
